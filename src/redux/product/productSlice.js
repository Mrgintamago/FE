...
export const getProduct = createAsyncThunk(
  ...
  async (payload) => {
    ...
    // Response structure after axios interceptor: { status, results, data: { data: [...], totalPage, currentPage } }
    // So response.data is { data: [...], totalPage, currentPage }
    const productsData = response.data || response;
    return {
      data: Array.isArray(productsData.data) ? productsData.data : (Array.isArray(productsData) ? productsData : []),
...
export const getProductSearch = createAsyncThunk(
  ...
  async (payload) => {
    ...
    let query = `limit=10&keyword=${payload.trim()}`;
    const response = await productApi.getAllProduct(query);
...
const productSlice = createSlice({
  ...
  extraReducers: {
    ...
    [getProduct.fulfilled]: (state, action) => {
      ...
      // action.payload is already { data: [...], totalPage, currentPage }
      state.product = Array.isArray(action.payload.data) 
        ? action.payload.data 
    ...
    [getProductId.fulfilled]: (state, action) => {
      ...
      state.productId = action.payload?.data?.data || action.payload?.data || {};
    },
    ...
    [getProductBrand.fulfilled]: (state, action) => {
      ...
      // API returns { status, data: { data: [...] } }
      state.productBrand = action.payload?.data?.data || [];
    },
